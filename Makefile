...
pkgdatadir = $(datadir)/merge
pkgincludedir = $(includedir)/merge
pkglibdir = $(libdir)/merge
pkglibexecdir = $(libexecdir)/merge
...
CONFIG_HEADER = $(top_builddir)/src/config/merge-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/merge-config.h.in \
...
ACLOCAL = ${SHELL} /home/merge/dev/merge/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/merge/dev/merge/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/merge/dev/merge/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/merge/dev/merge/build-aux/missing automake-1.15
AWK = mawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /home/merge/dev/merge/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = merge
PACKAGE_BUGREPORT = www.projectmerge.org
PACKAGE_NAME = Merge Core
PACKAGE_STRING = Merge Core 1.0.4
PACKAGE_TARNAME = merge
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /home/merge/dev/merge
abs_srcdir = /home/merge/dev/merge
abs_top_builddir = /home/merge/dev/merge
abs_top_srcdir = /home/merge/dev/merge
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/merge/dev/merge/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_MERGE_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_MERGE_coverage.info test_MERGE.info
...
src/config/merge-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/merge-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/merge-config.h
$(top_srcdir)/src/config/merge-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/merge-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/MERGE-Qt: all-recursive
	$(MKDIR_P) $(@D)
...
#test_MERGE.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_MERGE -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_MERGE_filtered.info: test_MERGE.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_MERGE_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_MERGE_coverage.info: baseline_filtered_combined.info test_MERGE_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_MERGE_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_MERGE_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_MERGE_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_MERGE.coverage/.dirstamp:  test_MERGE_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_MERGE.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_MERGE.coverage/ total.coverage/ $(OSX_APP)
